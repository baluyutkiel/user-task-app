  ...
  describe('filterTask', () => {
    ...
    it('should filter task by scheduledDate key', () => {
      ...
      service.tasks = [
        ...
        generateTask({ scheduledDate: futureDate }),
      ];
      service.filterTask('scheduledDate');
      expect(service.tasks.length).toEqual(1);
      expect(service.tasks[0].scheduledDate).toEqual(today);
    });
  });
  ...
  describe('searchTask', () => {
    ...
    it('should search task list for a fuzzy match on title', () => {
      service.tasks = [
        ...
        generateTask({ title: 'Complete take home assignment' }),
      ];
      service.searchTask('load');
      expect(service.tasks.length).toEqual(1);
      expect(service.tasks[0].title).toEqual('Load app');
    });    
  });
});
