...
describe('ListComponent', () => {
  ...
  it(`should not display archived tasks after deleting them`, async () => {
    jest.spyOn(component, 'onDeleteTask');  
    let taskCards = fixture.debugElement.queryAll(By.css('mat-card'));
    const initialTaskCount = taskCards.length;
    expect(initialTaskCount).toBeGreaterThanOrEqual(2); // Ensure at least 2 exist
    const deleteButton = await loader.getHarness(
      MatButtonHarness.with({ selector: '[data-testid="delete-task"]' })
    );
    await deleteButton.click();
    fixture.detectChanges();
    expect(component.onDeleteTask).toHaveBeenCalledTimes(1);
    taskCards = fixture.debugElement.queryAll(By.css('mat-card'));
  
    if (initialTaskCount > 2) {
      expect(taskCards.length).toBeLessThan(initialTaskCount);
    }
